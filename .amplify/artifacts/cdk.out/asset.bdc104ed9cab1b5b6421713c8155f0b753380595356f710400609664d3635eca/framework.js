...
async function onEvent(cfnRequest) {
  ...
  onEventResult?.NoEcho
    ? (0, util_1.log)(
        "redacted onEvent returned:",
        cfnResponse.redactDataFromPayload(onEventResult),
      )
    : (0, util_1.log)("onEvent returned:", onEventResult);
  const resourceEvent = createResponseEvent(cfnRequest, onEventResult),
    sanitizedEvent = { ...resourceEvent, ResponseURL: "..." };
  if (
    (onEventResult?.NoEcho
      ? (0, util_1.log)(
          "readacted event:",
          cfnResponse.redactDataFromPayload(sanitizedEvent),
        )
      : (0, util_1.log)("event:", sanitizedEvent),
    !process.env[consts.USER_IS_COMPLETE_FUNCTION_ARN_ENV])
  )
    return cfnResponse.submitResponse("SUCCESS", resourceEvent, {
      noEcho: resourceEvent.NoEcho,
    });
  const waiter = {
...
async function isComplete(event) {
  ...
  event?.NoEcho
    ? (0, util_1.log)(
        "redacted isComplete request",
        cfnResponse.redactDataFromPayload(sanitizedRequest),
      )
  ...
  if (
    (event?.NoEcho
      ? (0, util_1.log)(
          "redacted user isComplete returned:",
          cfnResponse.redactDataFromPayload(isCompleteResult),
        )
      : (0, util_1.log)("user isComplete returned:", isCompleteResult),
    !isCompleteResult.IsComplete)
  )
    throw isCompleteResult.Data && Object.keys(isCompleteResult.Data).length > 0
      ? new Error('"Data" is not allowed if "IsComplete" is "False"')
      : new cfnResponse.Retry(JSON.stringify(event));
  const response = {
    ...event,
    ...isCompleteResult,
    Data: { ...event.Data, ...isCompleteResult.Data },
  };
...
async function invokeUserFunction(
  ...
  (0, util_1.log)(
    `executing user function ${functionArn} with payload`,
    sanitizedPayload,
  );
  ...
  if (resp.FunctionError) {
    ...
    const errorMessage = jsonPayload.errorMessage || "error",
      arn = functionArn.split(":"),
      functionName = arn[arn.length - 1],
      message = [errorMessage, "", `Logs: /aws/lambda/${functionName}`, ""]
        .join(`
`),
      e = new Error(message);
    throw (
...
function createResponseEvent(cfnRequest, onEventResult) {
  ...
  return (
    ...
    { ...cfnRequest, ...onEventResult, PhysicalResourceId: physicalResourceId }
  );
}
...
module.exports = {
  ...
  [consts.FRAMEWORK_IS_COMPLETE_HANDLER_NAME]:
    cfnResponse.safeHandler(isComplete),
