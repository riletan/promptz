...
export function request(ctx) {
  ...
  const ruleId = id || util.autoId();

  // Create slug from name and ID
  const slug = `${slugify(name)}-${ruleId.split("-")[0]}`;
  ...
  if (isCreate) {
    return {
      operation: "PutItem",
      key: util.dynamodb.toMapValues({ id: ruleId }),
    ...
    return {
      ...
      update: {
        expression:
          "SET #name = :name, #slug = :slug, #description = :description, #content = :content, #tags = :tags, #public = :public, #sourceURL = :sourceURL, #updatedAt = :updatedAt",
        expressionNames: {
          "#name": "name",
          "#slug": "slug",
